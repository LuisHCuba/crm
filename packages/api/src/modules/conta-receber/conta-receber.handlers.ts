...
import {
  ...
  generateContaReceberSchema,
  receiveContaReceberSchema,
} from "./conta-receber.schemas";
...
function listWhereClause(query: Record<string, unknown>) {
  const filterConds = buildFilters(query, [
    { field: receivables.status, type: "eq", param: "status" },
    { field: receivables.dealId, type: "eq", param: "dealId" },
    { field: receivables.companyId, type: "eq", param: "companyId" },
    { field: receivables.categoryId, type: "eq", param: "categoryId" },
    { field: receivables.dueDate, type: "gte", param: "dueDateFrom" },
    { field: receivables.dueDate, type: "lte", param: "dueDateTo" },
  ]);
  const parts = [...filterConds];
  if (query.includeArchived !== "true") parts.unshift(notArchived(receivables));
...
export async function list(
  ...
  const [totals] = await db
    .select({
      ...
      totalPaid: sql<string>`COALESCE(SUM(CASE WHEN status = 'paid' THEN received_value ELSE 0 END), 0)`,
    })
    .from(receivables)
    .where(request.query.includeArchived === "true" ? undefined : notArchived(receivables));
...
export async function getById(
  ...
  try {
    ...
    const [row] = await db
      .select()
      .from(receivables)
      .where(and(eq(receivables.id, id), notArchived(receivables)))
      .limit(1);
...
export async function create(
  ...
  try {
    ...
    const parsed = createContaReceberSchema.safeParse(request.body);
  if (!parsed.success) {
  ...
  const inserted = await db.insert(receivables).values(valuesToInsert).returning();
...
export async function generate(
  ...
  try {
    const user = request.user as { id: string; email: string };
    const parsed = generateContaReceberSchema.safeParse(request.body);
  if (!parsed.success) {
    return reply.status(400).send({ error: "Validation", issues: parsed.error.issues });
  }
  ...
  for (const item of items) {
    const parcelGroup = crypto.randomUUID();
    const parcelValue = (Number(item.value) / item.parcels).toFixed(2);
    const valuesToInsert = [];

    for (let i = 0; i < item.parcels; i++) {
      valuesToInsert.push({
        description: `Parcela ${i + 1}/${item.parcels}`,
        companyId,
        dealId,
        productId: item.productId ?? null,
        value: parcelValue,
        dueDate: i === 0 ? item.firstDueDate : addMonths(item.firstDueDate, i),
        bankAccountId: item.bankAccountId ?? null,
        categoryId: item.categoryId ?? null,
        parcelGroup,
        parcelLabel: `${i + 1}/${item.parcels}`,
      });
    }
    ...
    return reply.status(201).send(allCreated);
  } catch (err) {
    return handleError(reply, err);
  }
}
...
export async function update(
  ...
  try {
    ...
    const parsed = updateContaReceberSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({ error: "Validation", issues: parsed.error.issues });
    }
    ...
    const [updated] = await db
      .update(receivables)
      .set(patch as Partial<InferInsertModel<typeof receivables>>)
...
export async function archive(
  ...
  try {
    ...
    await archiveRecord(receivables, id, user.id, "receivable");
...
export async function restore(
  ...
  try {
    ...
    const [existing] = await db
      .select({ id: receivables.id })
      .from(receivables)
      .where(and(eq(receivables.id, id), eq(receivables.archived, true)))
      .limit(1);
    ...
    await restoreRecord(receivables, id, user.id, "receivable");
...
export async function receive(
  ...
  try {
    ...
    const parsed = receiveContaReceberSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({ error: "Validation", issues: parsed.error.issues });
    }
    ...
    if (existing.status === "paid") {
      return reply.status(400).send({ error: "VALIDATION_ERROR", message: "Conta já recebida" });
    }
    ...
    const [updated] = await db
      ...
      .set({
        ...
        receivedValue: parsed.data.receivedValue,
        bankAccountId: parsed.data.bankAccountId,
      } as Partial<InferInsertModel<typeof receivables>>)
      .where(eq(receivables.id, id))
      .returning();
...
export async function cancel(
  ...
  try {
    ...
    const [updated] = await db
      .update(receivables)
      .set({ status: "cancelled" } as Partial<InferInsertModel<typeof receivables>>)
      .where(eq(receivables.id, id))
      .returning();
