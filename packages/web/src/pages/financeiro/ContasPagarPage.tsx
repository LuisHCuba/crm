...
import { ArrowUpCircle, Plus, RotateCcw } from "lucide-react";
...
export function ContasPagarPage() {
  ...
  const categoryOpts = useCategoryOptions("expense");
  ...
  const restoreMutation = useMutation({
    mutationFn: (id: string) => api.patch(`/contas-pagar/${id}/restore`),
    onSuccess: () => {
  ...
  const columns: DataTableColumn<Payable>[] = [
    { key: "description", header: "Descrição" },
    {
      key: "companyId",
      header: "Empresa",
      render: (row): React.ReactNode => (row.companyId ? companyMap.get(row.companyId) ?? "—" : "—"),
    },
    {
      key: "value",
      header: "Valor",
